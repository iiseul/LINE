$(function () {
    ...
    //'input', 'arrow', 'list' 전체를 변수 지정 후,
    //언어 리스트의 'a'선택 시 클릭한 text로 내용 변경
    //클릭이 이루어진 'a'의 부모인 'li'에 'on'클래스 생성, 나머지 형제들 'on'삭제
    //언어 리스트 클릭 시, 보기/가리기
    //'input'내 'arrow' 추가, 클릭 시 언어목록 보이면 숨기고, 숨겨져 있으면 나타나도록 처리
    //'arrow' 클릭 시 위를 향하는 'arrow'클래스 추가
    //마우스가 선택한 언어박스를 나갔을 때, 언어 리스트를 가리고 위를 향하는 'arrow' 삭제

    // .text() : text를 불러오거나 변경할 때 사용
    // .parent() : 부모 선택
    // .toggle() : 선택한 요소가 보이면 숨기고, 숨겨져있으면 나타남
    // .toggleClass() : 지정한 클래스가 없으면 생성, 있으면 제거
    // .val() : 'input'속성 값을 불러오거나 변경
    // .find() : 앞에 선택한 요소 내에서 지정한 요소를 다시 찾아 선택

    var $input = $("input#lang"); //input 박스
    var $arrow = $(".opt span.arrow"); //arrow
    var $langlist = $(".langlist"); //list 전체 박스

    $langlist.find("a").on("click", function () { // 'this' -> 클릭이 일어난 'a'
        $input.val($(this).text()); //'input'내 에서 클릭한 text로 변경
        $(this).parent().addClass("on").siblings().removeClass("on"); //.parent() -> 'a'의 부모 'li'
        $langlist.toggle();
    });
    ...
    var telText_MO = "듣고싶은 목소리, 보고싶은 얼굴이 있다면 <br> 망설이지마세요.";
    var stickerText_MO = "10,000개 넘는 스티커와 이모티콘으로 <br> 미묘한 감정까지 표현해보세요.";
    var shareText_MO = "사진, 동영상은 물론 <br> 음성 메세지와 연락처, 위치정보까지<br> 손쉽게 보낼 수 있습니다.";
    ...
    //메뉴클릭시 각 article 영역으로 애니메이션되며, 이동
